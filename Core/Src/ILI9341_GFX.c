...
void ILI9341_DrawChar(char ch, const uint8_t font[], uint16_t X, uint16_t Y, uint16_t color, uint16_t bgcolor) {
    // biggest char occupies this
	static uint8_t c[2*15*19] = {0};
	...
	for (int j = 0; j < fHeight; j++) {
		for (int i = 0; i < fWidth; i++) {
			...
			uint8_t b = 1 << (j & 0x07);
			if ((z & b) != 0x00) {
				c[0 + i*2 + j*2*fWidth] = color >> 8;
				c[1 + i*2 + j*2*fWidth] = color;
				/* ILI9341_DrawPixel(X+i, Y+j, color); */
			} else {
				c[0 + i*2 + j*2*fWidth] = bgcolor >> 8;
				c[1 + i*2 + j*2*fWidth] = bgcolor;
			}
		}
	}

	const uint16_t eX = X + fWidth-1;
	const uint16_t eY = Y + fHeight-1;
    ILI9341_SetAddress(X, Y, eX, eY);
	ILI9341_WriteMemory(c, sizeof(uint8_t) * fWidth * fHeight * 2); // COLOR
}
...
void ILI9341_DrawText(const char* str, const uint8_t font[], uint16_t X, uint16_t Y, uint16_t color, uint16_t bgcolor) {
	...
	/* ILI9341_DrawRectangle(X, Y, fWidth*len, fHeight, bgcolor); */
